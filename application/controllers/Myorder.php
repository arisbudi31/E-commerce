...
class Myorder extends MY_Controller {
    ...
    public function confirm($invoice){
        ...
        if($data['order']->status !== 'waiting'){
            $this->session->set_flashdata('warning', 'Bukti transfer berhasil dikirim');
        ...
        if(!empty($_FILES) && $_FILES['image']['name'] !== ''){
            $image_name = url_title($invoice, '-', true) . '-' . date('YmdHis');
            $upload = $this->myorder->uploadImage('image', $image_name);
            if($upload){
        ...
        if($this->myorder->create($data['input'])){
            ...
            $this->session->set_flashdata('error', 'Oopss! Terjadi suatu kesalahan');
